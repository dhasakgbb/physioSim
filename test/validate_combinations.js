...
const scenarios = [
  {
    name: "TRT Cruise",
    stack: [
      { compound: 'testosterone', dose: 150 }
    ]
  },
  {
  ...
  },
  {
    ...
    stack: [
      { compound: 'testosterone', dose: 1000 }
    ]
  },
  {
...
scenarios.forEach(scenario => {
  ...
  const { totalBenefit, totalRisk, netScore, saturationPenalty } = result.totals;
  ...
  console.log(`Stack: ${scenario.stack.map(s => `${s.dose}mg ${compoundData[s.compound].name}`).join(', ')}`);
  console.log(`-----------------------------------------`);
  console.log(`Benefit:    ${totalBenefit.toFixed(2)}`);
  console.log(`Risk:       ${totalRisk.toFixed(2)}`);
  console.log(`Net Score:  ${netScore.toFixed(2)}`);
  console.log(`Saturation: ${(saturationPenalty * 100).toFixed(0)}% efficiency`);
