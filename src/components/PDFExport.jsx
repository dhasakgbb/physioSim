    ...
    try {
      ...
      if (chartRef.current) {
        try {
          ...
          pdf.setFontSize(10);
          pdf.text(
            "Chart capture failed. Please try again.",
            margin,
            yPosition,
          );
        }
      ...
      if (stackData && stackData.compounds && stackData.compounds.length > 0) {
        ...
        pdf.setFont("helvetica", "normal");
        pdf.text(
          `Total Benefit Score: ${stackData.totalBenefit.toFixed(1)}`,
          margin + 3,
          yPosition,
        );
        yPosition += 5;
        pdf.text(
          `Total Risk Score: ${stackData.totalRisk.toFixed(1)}`,
          margin + 3,
          yPosition,
        );
        yPosition += 5;
        const benefitSynergyPercent =
          stackData.benefitSynergyPercent ??
          (stackData.totalBenefit > 0
            ? (stackData.benefitSynergy / stackData.totalBenefit) * 100
            : 0);
        const riskSynergyPercent =
          stackData.riskSynergyPercent ??
          (stackData.totalRisk > 0
            ? (stackData.riskSynergy / stackData.totalRisk) * 100
            : 0);
        pdf.text(
        ...
        const adjustedBenefit =
          stackData.totalBenefit + (stackData.benefitSynergy || 0);
        const adjustedRisk = stackData.totalRisk + (stackData.riskSynergy || 0);
        const ratio =
          adjustedRisk > 0
            ? (adjustedBenefit / adjustedRisk).toFixed(2)
            : "N/A";

        pdf.text(
          `Adjusted Benefit/Risk Ratio: ${ratio}`,
          margin + 3,
          yPosition,
        );
        yPosition += 10;
            ...
            for (let j = i + 1; j < stackData.compounds.length; j++) {
              ...
              if (comp1 && comp2) {
                const interaction = getInteraction(
                  stackData.compounds[i].id,
                  stackData.compounds[j].id,
                );
        ...
        // Ancillary Protocol
        if (
          stackData.ancillaryProtocol &&
          stackData.ancillaryProtocol.length > 0
        ) {
          ...
          stackData.ancillaryProtocol.forEach((anc) => {
            ...
            if (anc.weeklyCost) {
              pdf.text(
                `Weekly Cost: $${anc.weeklyCost.toFixed(2)}`,
                margin + 6,
                yPosition,
              );
              yPosition += 4;
          ...
          if (totalWeeklyCost > 0) {
            pdf.setFont("helvetica", "bold");
            pdf.text(
              `Total Estimated Weekly Cost: $${totalWeeklyCost.toFixed(2)}`,
              margin + 3,
              yPosition,
            );
            yPosition += 8;
