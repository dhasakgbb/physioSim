...
const SweetSpotFinder = ({
  ...
  const handleFind = () => {
    const applicable = Object.entries(compoundData).filter(
      ([key, data]) => data.type === compoundType && visibleCompounds[key],
    );
    ...
    const computed = applicable
      .map(([key]) => findSweetSpotRange(key, userProfile))
      .filter(Boolean);
    setResults(computed);
    setStatus("done");
  };
  ...
  return (
    <div
      className={
        inline ? "flex flex-col gap-2 flex-1 min-w-[220px]" : "min-w-[220px]"
      }
    >
      <div
        className={
      ...
      {results.length > 0 && (
        <div
          className={`grid grid-cols-1 gap-3 text-xs ${inline ? "pt-2 border-t border-dashed border-physio-bg-border/70" : "mt-3"}`}
        >
          {results.map((result) => {
            const doseWindow = deriveDoseWindow(result.compoundKey);
            const rangeStartPct =
              ((result.optimalRange[0] - doseWindow.min) /
                (doseWindow.max - doseWindow.min)) *
              100;
            const rangeWidthPct =
              ((result.optimalRange[1] - result.optimalRange[0]) /
                (doseWindow.max - doseWindow.min)) *
              100;
              ...
              >
                ...
                <div className="relative h-6 w-full bg-physio-bg-highlight rounded-full overflow-hidden">
                  ...
                  <div
                    ...
                    style={{
                      left: `${((result.peakDose - doseWindow.min) / (doseWindow.max - doseWindow.min)) * 100}%`,
                    }}
                  />
                  {/* Warning Marker */}
                  {result.warningDose && (
                    <div
                      ...
                      style={{
                        left: `${((result.warningDose - doseWindow.min) / (doseWindow.max - doseWindow.min)) * 100}%`,
                      }}
                    />
                  )}
                </div>
                ...
                <p className="text-physio-text-primary text-xs">
                  Optimal window:{" "}
                  <strong>
                    {formatRange(result.optimalRange)} {result.unit}
                  </strong>
                  {doseWindow?.min !== undefined &&
                    ` • Guardrail ${doseWindow.min}-${doseWindow.max} ${result.unit}`}
                </p>
                {result.warningDose && (
                  <p className="text-physio-accent-critical text-[11px]">
                    Above {result.warningDose} {result.unit} risk outpaces
                    benefit.
                  </p>
                )}
              </div>
