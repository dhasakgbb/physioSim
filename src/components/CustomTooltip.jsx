...
const CustomTooltip = ({ active, payload, viewMode }) => {
  ...
  const compoundsToShow = payload
    .filter(
      (p) =>
        p.dataKey &&
        !p.dataKey.includes("upper") &&
        !p.dataKey.includes("lower"),
    )
    .map((p) => {
  ...
  return (
    <div className="bg-physio-bg-secondary/95 backdrop-blur-sm border-2 border-physio-accent-cyan/40 rounded-lg p-4 shadow-physio-elevated max-w-md">
      <div className="font-bold text-physio-text-primary mb-2 font-mono">
        Dose: {dose} mg/week
      </div>
      ...
      {compoundsToShow.map(({ key, type }) => {
        ...
        const curve =
          type === "benefit" ? compound.benefitCurve : compound.riskCurve;
        ...
        const showRisk =
          ...
          viewMode === "uncertainty";
        const shouldShow =
          (type === "benefit" && showBenefit) || (type === "risk" && showRisk);
        ...
        return (
          <div
            key={`${key}-${type}`}
            className="mb-4 last:mb-0 pb-4 last:pb-0 border-b last:border-b-0 border-physio-bg-border"
          >
            <div
            ...
            <div className="space-y-1 text-sm text-physio-text-primary">
              <div className="flex items-center">
                <span className="font-semibold mr-2 text-physio-text-secondary">
                  ★ Score:
                </span>
              ...
              <div className="flex items-start">
                <span className="font-semibold mr-2 whitespace-nowrap text-physio-text-secondary">
                  Tier:
                </span>
              ...
              <div className="flex items-start">
                ...
                <span className="flex-1 text-xs text-physio-text-secondary">
                  {point.source || "N/A"}
                </span>
              </div>
