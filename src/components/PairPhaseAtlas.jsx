...
const hexToRgb = (hex = '#ffffff') => {
  ...
  const value = parseInt(normalized.length === 3 ? normalized.repeat(2) : normalized, 16) || 0xffffff;
  return {
    r: (value >> 16) & 255,
    g: (value >> 8) & 255,
    b: value & 255
...
const resolvePhaseCoords = (phase, rangeA, rangeB) => {
  if (phase.doses) {
    const normA = phase.doses.a != null ? normalizeValue(phase.doses.a, rangeA) : undefined;
    const normB = phase.doses.b != null ? normalizeValue(phase.doses.b, rangeB) : undefined;
...
const PairPhaseAtlas = ({ pair, surfaceData = [], doseRanges = {} }) => {
  ...
  const rangeA = doseRanges[compoundA] || pair.doseRanges?.[compoundA] || [0, 1000];
  const rangeB = doseRanges[compoundB] || pair.doseRanges?.[compoundB] || [0, 1000];
  ...
  const subtitle = atlasMeta.subtitle || 'Use the normalized ridge to map your real-world experiments.';
  const description = atlasMeta.description ||
  ...
  return (
    <section className="bg-physio-bg-secondary border border-physio-bg-border rounded-3xl p-5 space-y-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wide text-physio-text-tertiary">{subtitle}</p>
          <h3 className="text-xl font-bold text-physio-text-primary">{title}</h3>
        ...
        <div className="text-xs text-physio-text-tertiary bg-physio-bg-core border border-physio-bg-border rounded-full px-3 py-1 text-center">
          {labelA}: {formatDoseRangeLabel(compoundA, rangeA)} · {labelB}: {formatDoseRangeLabel(compoundB, rangeB)}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <PhaseSurface
          ...
          caption={atlasMeta.benefitCaption || 'Golden mist = total benefit. Slide along P1 → P3 for the biggest ROI.'}
        ...
        <PhaseSurface
          title="Risk surface"
          caption={atlasMeta.riskCaption || 'Orange flare = cumulative risk. The P4 corner is the cliff—treat it as expensive time.'}
          points={riskPoints}
          maxValue={maxRisk}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {phases.map(phase => (
          <div key={phase.id} className="p-4 rounded-2xl border border-physio-bg-border bg-physio-bg-core">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-semibold text-physio-text-primary">{phase.title}</h4>
            </div>
            <p className="text-xs text-physio-text-secondary mt-2">{phase.summary}</p>
      ...
      <p className="text-[11px] text-physio-text-tertiary text-center">
        Conceptual only. Normalize your logs into these phases, monitor labs, and treat high-phase time as structurally expensive.
      </p>
    </section>
  );
};

const PhaseSurface = ({ title, caption, points, maxValue, color, phases, labelA, labelB }) => (
  <div className="bg-physio-bg-core border border-physio-bg-border rounded-2xl p-4 flex flex-col gap-3">
    <div>
      <h4 className="text-sm font-semibold text-physio-text-primary">{title}</h4>
