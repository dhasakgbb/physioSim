...
const deriveHeavyTrainingScore = (profile) => {
  const weightComponent = clamp(
    ((profile.bodyweight || defaultProfile.bodyweight) - 85) / 40,
    0,
    1,
  );
  const trainingComponent = clamp(
    ((profile.yearsTraining || defaultProfile.yearsTraining) - 3) / 9,
    0,
    1,
  );
...
const getScale = (profile, key) =>
  profile?.curveScales?.[key] ?? defaultCurveScales[key] ?? 1;
...
export const personalizeScore = ({
  ...
  if (
    anxietySensitiveCompounds.has(compoundKey) &&
    curveType === "risk" &&
    anxietyDelta !== 0
  ) {
  ...
  const personalizedCi = clamp(
    ciFloor * ciMultiplier * uncertaintyScale,
    0,
    1.5,
  );
...
export const buildPersonalizationNarrative = (profile = defaultProfile) => {
  ...
  if (heavyTrainingScore > 0.4) {
    talkingPoints.push(
      "Heavy training load recognized — benefit curves start ~0.2-0.5 higher.",
    );
  }

  if (shbgDelta > 0.25) {
    talkingPoints.push(
      "High SHBG detected — Testosterone benefit curve shifts right (needs more mg for same effect).",
    );
  } else if (shbgDelta < -0.25) {
    talkingPoints.push(
      "Low SHBG flagged — Testosterone potency boosted; watch E2 even at “low” doses.",
    );
  }
  ...
  if ((profile.experience || "test_only") === "none") {
    talkingPoints.push(
      "No prior AAS exposure — benefit inflated but risk penalty applied across compounds.",
    );
  } else if (profile.experience === "blast_cruise") {
    talkingPoints.push(
      "Blast/cruise veteran — marginal benefit dampening assumed due to desensitization.",
    );
  }

  if (!talkingPoints.length) {
    talkingPoints.push(
      "Baseline responder template — curves mirror published data closely.",
    );
  }
